...
def createModel(modelParams):
  ...
  model.enableInference({"predictedField": "hourly_traffic_count1"})
  return model
...
def runIoThroughNupic(inputData, model, gymName, plot):
  ...
  for row in csvReader:
    ...
    result = model.run({
      "timestamp": timestamp,
      "hourly_traffic_count1": float(row[1]),
      "hourly_traffic_count2": float(row[2])
    })
